...
public class RoundIndicatorView extends View {
    ...
    private void drawIndicator(Canvas canvas) {
        ...
        int w = dp2px(10);//与开始一致，外圆距离内圆10dp
    ...
    private void drawScale(Canvas canvas) {
        canvas.save();
        float angle = (float) sweepAngle / 30;//算出小刻度的弧度
        canvas.rotate(90 + startAngle); //将起始刻度点旋转到正上方（270)
        for (int i = 0; i <= 30; i++) {//有30个小刻度
            if (i % 6 == 0) {   //画粗刻度和刻度值
...
//        paint.getTextBounds(text,0,text.length(),rect);
    ...
    private void drawRound(Canvas canvas) {
        ...
        //第三个参数是true是使用中心点的意思，中心点会连接起点和终点
